...
fn cs(@builtin(global_invocation_id) cell:vec3u)  {
  ...
  var forward = vec2f( 0., 1. ); //up
  let d = fract( vant.dir );
  if( d == 0.25 ) {
    forward = vec2f( 1., 0. ); //right
  } else if( d == 0.5 ) {
    forward = vec2f( 0., -1. ); //down
  } else {
  ...
  // if pheromone in fron and not in back 
  if ( pheremones[ frontIndex ] == 1. && pheremones[ backIndex ] == 0. ) {
    pheremones[ frontIndex ] = 0.; //flip pheromone flag
    pheremones[ backIndex ] = 1.;
  }

  // if pheromones were found
  if( pheromone != 0. ) {
    pheremones[ pIndex ] = 0.;  // set pheromone flag
    //if ( pheremones[ frontIndex ] == 0. && pheremones[ backIndex ] == 1. ) {
    //    vant.dir += .5; // turn 180 degrees
    //} else {
        vant.dir += select(.25,-.25,vant.flag==0.); // turn 90 degrees 
    //}
  }else{
